...
# CONFIGURAÇÕES
# ─────────────────────────────────────────────
QR_VALIDADE_SEGUNDOS   = 20    # QR expira em 20 s (ajuste se quiser mais tempo)
LIMITE_DISTANCIA_METROS = 100  # raio anti-fraude em metros (calibrar após testes de campo)
...
def criar_tabelas():
    ...
    # Colunas que podem faltar em bancos antigos
    ...
    if request.method == "POST":
        ...
        if user:
            if user["tipo"] != tipo_form:
                ...
                return redirect("/dashboard" if user["tipo"] == "motorista" else "/dashboardusuario")
        else:
...
def dashboard():
    ...
    pontos_mapa = conn.execute("""
        SELECT u.nome, u.ponto, u.linha,
               u.lat_residencia AS lat,
               u.lng_residencia AS lng
        FROM usuarios u
    ...
    historico = conn.execute("""
        ...
        FROM presencas p
        WHERE p.motorista_id = ?
          AND p.data_hora >= date('now','-6 days')
        GROUP BY dia
        ORDER BY dia
...
# ─────────────────────────────────────────────
# DASHBOARD DO USUÁRIO / PASSAGEIRO
# ─────────────────────────────────────────────
@app.route("/dashboardusuario")
def dashboardusuario():
    ...
    # Histórico de presenças do próprio usuário (últimos 10)
    historico = conn.execute("""
        SELECT strftime('%d/%m/%Y', p.data_hora) AS data,
               ...
               u.nome                             AS motorista
        FROM presencas p
        JOIN usuarios u ON u.id = p.motorista_id
        WHERE p.usuario_id = ?
        ORDER BY p.data_hora DESC
        LIMIT 10
...
# Compatibilidade com link antigo no dashboard.html
...
def registrar():
    ...
    if datetime.now() > datetime.fromisoformat(qr["expira_em"]):
        conn.close()
        return jsonify({"status": "erro", "msg": "QR Code expirado. Aguarde o motorista atualizar."}), 400
    ...
    # 4. Validação de distância (se GPS disponível)
    if lat_aluno is not None and lng_aluno is not None:
        ...
        if loc is None:
            ...
            return jsonify({
                "status": "erro",
                "msg": "GPS do motorista indisponível. Peça para o motorista ativar o GPS."
            }), 400
    ...
    # 7. Grava presença com prova de localização
    conn.execute("""
...
# ─────────────────────────────────────────────
# SALVAR LOCALIZAÇÃO RESIDENCIAL
# ─────────────────────────────────────────────
